#!/usr/bin/env python3
"""
    subDomainsBrute 1.3 (2019-11-02 For python3.)
...
class SubNameBrute(object):
    def __init__(self, *params):
        ...
        self.outfile = open('{}/{}_part_{}.txt'.format(tmp_dir, self.domain, self.process_num), 'w')
...
def wildcard_test(domain, level=1):
    try:
        ...
        answers = r.query('lijiejie-not-existed-test.{}'.format(domain))
        ips = ', '.join(sorted([answer.address for answer in answers]))
        if level == 1:
            print ('any-sub.{}\t{}'.format(domain.ljust(30), ips))
            wildcard_test('any-sub.{}'.format(domain, 2))
        elif level == 2:
            exit(0)
    except Exception as e:
        return domain
...
def get_sub_file_path():
    if options.full_scan and options.file == 'subnames.txt':
        ...
        if os.path.exists(options.file):
            path = options.file
        elif os.path.exists('dict/{}'.format(options.file)):
            path = 'dict/{}'.format(options.file)
        else:
            print_msg('[ERROR] Names file not found: {}'.format(options.file))
            exit(-1)
    return path
...
if __name__ == '__main__':
    ...
    print ('''  
SubDomainsBrute v1.3 (2019-11-02 For python3.)
Reference source: https://github.com/lijiejie/subDomainsBrute (2019-05-19 python2.)
           ''')
    # make tmp dirs
    tmp_dir = 'tmp/{}_{}'.format(args[0], int(time.time()))
    ...
    try:
        ...
        print ('[+] Start {} scan process'.format(options.process))
        ...
        while all_process:
            ...
            for c in queue_size_array:
                groups_count += c
            msg = '[{}] {} found, {} scanned in {:.1f} seconds, {} groups left'.format(
    ...
    msg = 'All Done. {} found, {} scanned in {:.1f} seconds.'.format(
        domain_count, scan_count.value, time.time() - start_time)
    print_msg(msg, line_feed=True)
    print ('Output file is {}'.format(out_file_name))
