...
def test_server(server, dns_servers):
    ...
    try:
        ...
        try:
            resolver.query('test.bad.dns.nonexisted.com')    # non-existed domain
            with open('bad_dns_servers.txt', 'a') as f:
                f.write(server + '\n')
            print_msg('[+] Bad DNS Server found {}'.format(server))
        except Exception as e:
            dns_servers.append(server)
        print_msg('[+] Server {} < OK >   Found {}'.format(server.ljust(16), len(dns_servers)))
    except Exception as e:
        print_msg('[+] Server {} <Fail>   Found {}'.format(server.ljust(16), len(dns_servers)))
...
def load_dns_servers():
    ...
    print_msg('\n[+] {} DNS Servers found'.format(server_count), line_feed=True)
    if server_count == 0:
